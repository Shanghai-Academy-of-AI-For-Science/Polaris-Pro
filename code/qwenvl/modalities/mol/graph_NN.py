...
# The GNN architecture and pretrained weights derive from the "Suiren" molecular
# graph model (released here under Apache-2.0 with the authors' permission).
...
from .org_mol2d import from_smiles

# Elements with atomic number < 100 (H through Es / Z=99) are allowed; this
# matches the size of ``x_embedding1`` (NUM_ATOM_TYPE=100 rows).
# Z >= 100 (Fm/Md/No/Lr/transactinides) are rejected at the
# build_graph filter (would index past x_embedding1's 100 rows and cause
# CUDA illegal memory access).  They are not chemically meaningful for
# any biological / pharmaceutical dataset.
ALLOWED_ELEMENTS = set(range(1, 100))
...
def get_suiren_arch_pretrained():
    return GNN( # these param are preset by suiren team
        num_layer=12, 
        emb_dim=256,
        drop_ratio=0.0,
