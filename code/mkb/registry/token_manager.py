...
def _collect_package_token_defs() -> Dict[str, Dict[str, str]]:
    """Collect TOKEN_DEFS from all modality packages under mkb.modalities.*."""
    import importlib
    import pkgutil
    collected: Dict[str, Dict[str, str]] = {}
    try:
        import mkb.modalities as _root
        for _imp, pkg_name, is_pkg in pkgutil.iter_modules(_root.__path__):
            if not is_pkg:
                continue
            try:
                mod = importlib.import_module(f"mkb.modalities.{pkg_name}")
            except Exception:
                continue
...
def get_all_known_modality_names() -> List[str]:
    ...
    declared by per-modality packages under ``mkb.modalities.*``.  Used to
